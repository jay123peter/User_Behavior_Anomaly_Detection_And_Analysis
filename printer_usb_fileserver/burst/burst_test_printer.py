...
##############################################
####file server to printer burst detection####
##############################################
...
log_printer_path = "./merge_printer"
...
#match file server burst time (1hour) to printer log(1day)#
def match_burst(burst_name, burst_file_server_df, log_file_server_df, log_printer_df):
    ...
    for i in burst_file_server_df['datetime']:
        ...
        #burst time add one day for printer log#
        start_i = i
    ...
    log_printer_df['Time'] = pd.to_datetime(log_printer_df['Time'])      
    ...
    for i,x in zip(start_end_file_server_time_day,start_end_file_server_time_hour):
        ...
        extract_datetime_printer_df = log_printer_df[log_printer_df['Time'].between(i[0],i[1])].copy()
        
        tmp_fileserver_filename_list = extract_datetime_fileserver_df["src_filename"].tolist()
        tmp_printer_filename_list = extract_datetime_printer_df["UUIDfileNM"].tolist()
        
        #if printer log empty represent no any log#
        if not tmp_printer_filename_list:
            continue
        
        #return jaccard_similarity score and intersection file name set#
        score,intersection_file_name_set = jaccard_similarity(tmp_printer_filename_list,tmp_fileserver_filename_list)
    ...
    df_printer_burst_time = pd.DataFrame(data = list_time_end, columns=['burst_time_end'])
    ...
    frames = [df_user_name,df_burst_name,df_file_server_burst_time,df_printer_burst_time,df_jaccard_score,df_warning,df_intersection_quantity,df_intersection_file_name]
...
#printer#
for path, folder, files in walk(log_printer_path):
    for file_name in files:
        log_printer_file_name.append(file_name)
       
#printer log and file server log intersection file name#
log_set_file_server_file_name = set(log_file_server_file_name)
log_set_printer_file_name = set(log_printer_file_name)
log_list_file_name_intersection = list(log_set_file_server_file_name.intersection(log_set_printer_file_name))

print("log file server data total user:",len(log_set_file_server_file_name))
print("log printer data: total user:",len(log_set_printer_file_name))
...
for extract_file_name, log_file_name in zip(extract_list_file_name_intersection,log_list_file_name_intersection): 
    ...
    log_printer_all_path = log_printer_path +'/'+log_file_name
    log_printer_df = pd.read_csv(log_printer_all_path, encoding = "utf-8-sig",low_memory=False)
    ...
    #####detection busrt#####
    file_server_Security_A_q,file_server_Security_A_weighted_bursts = burst('fileserver_Security_A', numpy_file_server_Security_A_r, numpy_file_server_d, file_server_Security_A_n, s=1.5, gamma=0.1, smooth_win=1, list_tmp_datetime = list_fileserver_datetime)
    ...
    ############## match log##############
    Security_A_df_match_burst = match_burst("Security_A_burst",burst_file_server_Security_A_df,log_file_server_df,log_printer_df)
    Security_B_df_match_burst = match_burst("Security_B_burst",burst_file_server_Security_B_df,log_file_server_df,log_printer_df)
    Security_C_df_match_burst = match_burst("Security_C_burst",burst_file_server_Security_C_df,log_file_server_df,log_printer_df)
    Security_D_df_match_burst = match_burst("Security_D_burst",burst_file_server_Security_D_df,log_file_server_df,log_printer_df)
    Security_none_df_match_burst = match_burst("Security_none_burst",burst_file_server_Security_none_df,log_file_server_df,log_printer_df)
