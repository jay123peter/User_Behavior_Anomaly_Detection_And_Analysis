...
def burst_detection(r,d,n,s,gamma,smooth_win=1):
    ...
    if smooth_win > 1:
        ...
        #modify#      #temp_p = temp_p.rolling(window=smooth_win, center=True).mean()
        temp_p = pd.Series(temp_p)
        temp_p = temp_p.rolling(window=3).mean()
        temp_p = temp_p.values
        #modify#
        
        #update r to reflect the smoothed proportions
        #modify# 
        r = temp_p*r        #r = temp_p*d
        d = temp_p*d
        r = r[~np.isnan(r)]
        d = d[~np.isnan(d)]
        #modify# 
    ...
    for t in range(int((smooth_win-1)/2),(int((smooth_win-1)/2))+real_n):
        ...
        #modify#             #q[t] = np.where(cost[t,:] == min(cost[t,:]))
        if cost[t,0] == -0. or cost[t,1] == -0.:
            q[np.isnan(q)] = 0
        else:
...
def enumerate_bursts(q, label):
    bursts = pd.DataFrame(columns=['label','begin_slot','end_slot','weight'])
    b = 0
    burst = False
    for t in range(1,len(q)):

        if (burst==False) & (q[t] > q[t-1]):
            bursts.loc[b,'begin_slot'] = t
            burst = True

        if (burst==True) & (q[t] < q[t-1]):
            bursts.loc[b,'end_slot'] = t
            burst = False
            b = b+1
...
def burst_weights(bursts, r, d, p):
    ...
    for b in range(len(bursts)):
        ...
        for t in range(bursts.loc[b,'begin_slot'], bursts.loc[b,'end_slot']):
